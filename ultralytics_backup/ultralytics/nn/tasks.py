...
from ultralytics.change_model.starnet import C2f_StarsBlock 
from ultralytics.change_model.eCBAM import C2f_eCBAM, C2f_LKALite, C2f_MultiBranchLKALite, C2f_ProgressiveLKALite, ResVANBlock
...
def parse_model(d, ch, verbose=True):
    ...
    base_modules = frozenset(
        {
            ...
            C2f_eCBAM,
            C2f_LKALite,
            C2f_MultiBranchLKALite,
            C2f_ProgressiveLKALite,
            ResVANBlock,
        }
    )
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            # Allow 'f' to be either an int (single source) or a list (multiple sources)
            # Older/complex YAMLs may use a list of source indices; handle both cases.
            if isinstance(f, list):
                # When multiple source indices are provided, modules receive the
                # concatenation of those feature maps, so c1 is the summed channels.
